...
{
  if ((startVirtualAddress + 1) > _EE_SIZE)
    return false;
  for (uint32_t i = startVirtualAddress; i < 1 + startVirtualAddress; i++)
  {
    if (data != NULL)
    {
...
//##########################################################################################################
bool write(uint32_t startVirtualAddress, uint8_t* data)
{
#if (_EE_USE_RAM_BYTE > 0)
  if ((startVirtualAddress + 1) > _EE_USE_RAM_BYTE)
    return false;
  if (data == NULL)
    return false;
  memcpy(&ee_ram[startVirtualAddress], data,1);
  return true;
#else
  return false;
#endif
